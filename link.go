...
func (link *Link) Kind() d.Kind {
	return LinkKind
}

// just returns itself for now
func (link *Link) Concerned() []d.ID {
	foo := make([]d.ID, 1)
	foo[0] = link.ID()
	return foo
}

func (link *Link) SetID(id d.ID) {
	link.Id = id.String()
}

func (link *Link) ID() d.ID {
	return d.ID(link.Id)
}
...
func (link *Link) Class(store d.Store) (*Class, error) {
	if link.ClassID == "" {
...
func (link *Link) RelationsIter(store d.Store) (d.ModelIterator, error) {
	// not yet completely general
	return mongo.NewIDIter(mongo.NewIDSetFromStrings(link.RelationsIDs), store), nil
}

func (link *Link) Relations(store d.Store) ([]*Relation, error) {
