...
func (link *Link) Class(db data.DB) (*Class, error) {
	if link.ClassID == "" {
...
func (link *Link) RelationsIter(db data.DB) (data.Iterator, error) {
	// not yet completely general
	return mongo.NewIDIter(mongo.NewIDSetFromStrings(link.RelationsIDs), db), nil
}

func (link *Link) Relations(db data.DB) ([]*Relation, error) {

	relations := make([]*Relation, 0)
	iter := mongo.NewIDIter(mongo.NewIDSetFromStrings(link.RelationsIDs), db)
