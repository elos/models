...
func (routine *Routine) Kind() d.Kind {
	return RoutineKind
}

// just returns itself for now
func (routine *Routine) Concerned() []d.ID {
	foo := make([]d.ID, 1)
	foo[0] = routine.ID()
	return foo
}

func (routine *Routine) SetID(id d.ID) {
	routine.Id = id.String()
}

func (routine *Routine) ID() d.ID {
	return d.ID(routine.Id)
}
...
func (routine *Routine) ActionsIter(store d.Store) (d.ModelIterator, error) {
	// not yet completely general
	return mongo.NewIDIter(mongo.NewIDSetFromStrings(routine.ActionsIDs), store), nil
}

func (routine *Routine) Actions(store d.Store) ([]*Action, error) {
...
func (routine *Routine) CompletedTasksIter(store d.Store) (d.ModelIterator, error) {
	// not yet completely general
	return mongo.NewIDIter(mongo.NewIDSetFromStrings(routine.CompletedTasksIDs), store), nil
}

func (routine *Routine) CompletedTasks(store d.Store) ([]*Task, error) {
...
func (routine *Routine) CurrentAction(store d.Store) (*Action, error) {
...
func (routine *Routine) TasksIter(store d.Store) (d.ModelIterator, error) {
	// not yet completely general
	return mongo.NewIDIter(mongo.NewIDSetFromStrings(routine.TasksIDs), store), nil
}

func (routine *Routine) Tasks(store d.Store) ([]*Task, error) {
...
func (routine *Routine) User(store d.Store) (*User, error) {
	if routine.UserID == "" {
		return nil, ErrEmptyLink
	}
