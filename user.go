...
func (user *User) IncludeAttribute(attribute *Attribute) {
	user.AttributesIDs = append(user.AttributesIDs, attribute.ID().String())
}

func (user *User) ExcludeAttribute(attribute *Attribute) {
	tmp := make([]string, 0)
	id := attribute.ID().String()
	for _, s := range user.AttributesIDs {
		if s != id {
			tmp = append(tmp, s)
		}
	}
	user.AttributesIDs = tmp
}

func (user *User) AttributesIter(db data.DB) (data.Iterator, error) {
	// not yet completely general
	return mongo.NewIDIter(mongo.NewIDSetFromStrings(user.AttributesIDs), db), nil
}

func (user *User) Attributes(db data.DB) ([]*Attribute, error) {

	attributes := make([]*Attribute, 0)
	iter := mongo.NewIDIter(mongo.NewIDSetFromStrings(user.AttributesIDs), db)
...
func (user *User) GetBSON() (interface{}, error) {
	...
	return struct {
		...
		AttributesIDs: user.AttributesIDs,
...
func (user *User) SetBSON(raw bson.Raw) error {
	...
	user.AttributesIDs = tmp.AttributesIDs
