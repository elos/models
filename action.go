...
func (action *Action) Kind() d.Kind {
	return ActionKind
}

// just returns itself for now
func (action *Action) Concerned() []d.ID {
	foo := make([]d.ID, 1)
	foo[0] = action.ID()
	return foo
}

func (action *Action) SetID(id d.ID) {
	action.Id = id.String()
}

func (action *Action) ID() d.ID {
...
func (action *Action) Actionable(store d.Store) (Actionable, error) {
	if action.ActionableID == "" {
		return nil, ErrEmptyLink
	}

	m := ModelFor(d.Kind(action.ActionableKind))
...
func (action *Action) Task(store d.Store) (*Task, error) {
	if action.TaskID == "" {
		return nil, ErrEmptyLink
	}
...
func (action *Action) User(store d.Store) (*User, error) {
	if action.UserID == "" {
		return nil, ErrEmptyLink
	}
