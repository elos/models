...
func (task *Task) Kind() d.Kind {
	return TaskKind
}

// just returns itself for now
func (task *Task) Concerned() []d.ID {
	foo := make([]d.ID, 1)
	foo[0] = task.ID()
	return foo
}

func (task *Task) SetID(id d.ID) {
	task.Id = id.String()
}

func (task *Task) ID() d.ID {
	return d.ID(task.Id)
}
...
func (task *Task) TaskDependenciesIter(store d.Store) (d.ModelIterator, error) {
	// not yet completely general
	return mongo.NewIDIter(mongo.NewIDSetFromStrings(task.TaskDependenciesIDs), store), nil
}

func (task *Task) TaskDependencies(store d.Store) ([]*Task, error) {
...
func (task *Task) User(store d.Store) (*User, error) {
	if task.UserID == "" {
